...
const successMessage = document.querySelector(".success"); // question accordian
...
signInForm.addEventListener("submit", e => {
  ...
  const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/; // Username Validation
...
signUpForm.addEventListener("submit", e => {
  ...
  let message = []; // Username Validation

  if (!usernameRegex.test(username)) message.push("Username must be a minimum 7 characters"); // Email Validation

  if (!emailRegex.test(email)) message.push("Email is not valid"); // Password Validation

  if (!passwordRegex.test(password)) message.push("Password must be a minimum 8 characters, at least 1 letter and 1 number"); // Confirm Password Validation

  if (!passwordRegex.test(confirmPassword)) {
    message.push("Password must be a minimum 8 characters, at least 1 letter and 1 number");
  ...
  if (!usernameRegex.test(username) || !emailRegex.test(email) || !passwordRegex.test(password) || !passwordRegex.test(confirmPassword) || password !== confirmPassword || !checkbox) {
    errorMessage.classList.remove("success");
    errorMessage.classList.remove("hidden");
  } else {
    errorMessage.textContent = "You have successfully signed up to Animestudio+";
