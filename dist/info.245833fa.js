...
exports.getCard = exports.SESSION_STORAGE_KEY = void 0;
...
const SESSION_STORAGE_KEY = "ANIME-SHOW-info"; // const infoPage = document.querySelector("[data-info-page]");
...
const getCard = async () => {
  try {
    ...
    if (response.ok) {
      ...
      const anime = data.data.documents.slice(0, 20); //   console.log(anime);
      ...
      buttons.forEach(btn => {
        btn.addEventListener("click", e => {
          ...
          const id = parseInt(e.target.dataset.id); //   console.log(id);
          ...
          const result = anime.find(show => show.id === id); // Storing the anime data in session storage but have to use JSON.stringify() the result we get and then we can use it on the other page and JSON.parse() it and use it in a new function to create a template with the info in sessionstorage.

          sessionStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(result)); // console.log(result);
...
exports.animeInfoTemplate = void 0;

var _grid = require("./grid.js");

const infoPage = document.querySelector("[data-info-page]"); // const info = JSON.parse(sessionStorage.getItem("ANIME-SHOW-info"));

const info = JSON.parse(sessionStorage.getItem(_grid.SESSION_STORAGE_KEY));
console.log(info);
...
exports.animeInfoTemplate = animeInfoTemplate;
